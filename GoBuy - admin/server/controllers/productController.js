...
exports.editProduct = (req, res) => {
    ...
    if (req.files.image2) {
        ...
        optionalQuery2 = `update product_images set product_image = '${req.files.image2[0].filename}' where product_id = ${id} order by image_id desc limit 1`;
    }

    // if (req.files.image1 && req.files.image2) {
    //     console.log("Two image uploaded");
    //     optionalQuery1 = `update product_images set product_image = '${req.files[0].filename}' where product_id = ${id} order by image_id limit 1`;
    //     optionalQuery2 = `update product_images set product_image = '${req.files[1].filename}' where product_id = ${id} order by image_id desc limit 1`;
    // }
    ...
    const query = `
    update products set title = ?, price = ? , description = ?, category = ?, rating_rate = ?, rating_count = ?, sku = ? where id = ?;
    ${optionalQuery1};
    ${optionalQuery2};
    `;
    const values = [
